...
using ADPasswordSecureCache.Encryption;
...
namespace ADPasswordSecureCache
{
    ...
    public class DiskCache<TKey> : IDiskCache<TKey>, IDiskCacheAsync<TKey> where TKey : IEquatable<TKey>
    {
        ...
        /// <summary>
        /// Gets the secure string value associated with a key.
        /// </summary>
        /// <param name="key">The key to locate in the cache.</param>
        /// <returns>A secure string from the cache.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is <c>null</c>.</exception>
        public SecureString GetSecureString(TKey key)
        {
            if (IsNull(key))
                throw new ArgumentNullException(nameof(key));
            if (!ContainsKey(key))
            ...
            SecureString decipheredSecureString = stringEncryptor.Decrypt(File.OpenRead(path));

            return decipheredSecureString;
        }
        ...
        /// <summary>
        /// Stores a secure string value associated with a key.
        /// </summary>
        /// <param name="key">The key used to locate the value in the cache.</param>
        /// <param name="value">SecureString to store in the cache.</param>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is <c>null</c> or <paramref name="value"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentException"><paramref name="value"/> is not readable.</exception>
        public void SetSecureString(TKey key, SecureString value)
        {
            if (IsNull(key))
                throw new ArgumentNullException(nameof(key));
            if (value == null)
                throw new ArgumentNullException(nameof(value));
            if (value.Length==0)
                throw new ArgumentException("The given secure sting is empty.", nameof(value));

            StringEncryptor stringEncryptor = new StringEncryptor();

            Stream cipheredStream = stringEncryptor.Encrypt(value);

            var result = SetValueCore(key, cipheredStream);
            switch (result)
            {
                case SetStatus.DataTooLarge:
                    throw new ArgumentException("The given stream received data that was larger than the allotted storage capacity of " + MaximumStorageCapacity.ToString(CultureInfo.InvariantCulture), nameof(value));
        ...
        /// </summary>
        /// <param name="key">The key used to locate the value in the cache.</param>
        /// <param name="value">A secure string to store in the cache.</param>
        /// <returns><c>true</c> if the data was able to be stored without error; otherwise <c>false</c>.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is <c>null</c> or <paramref name="value"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentException"><paramref name="value"/> is not readable.</exception>
        public bool TrySetSecureString(TKey key, SecureString value)
        {
            if (IsNull(key))
                throw new ArgumentNullException(nameof(key));
            if (value == null)
                throw new ArgumentNullException(nameof(value));
            if (value.Length == 0)
                throw new ArgumentException("The given secure string is empty.", nameof(value));

            StringEncryptor stringEncryptor = new StringEncryptor();

            Stream cipheredStream = stringEncryptor.Encrypt(value);

            return SetValueCore(key, cipheredStream) == SetStatus.Success;
        }
        ...
        /// </summary>
        /// <param name="key">The key to locate in the cache.</param>
        /// <param name="stream">A secure string from the cache. Will be <c>null</c> when <paramref name="key" /> does not exist in the cache.</param>
        /// <returns><c>true</c> if the cache contains the key; otherwise <c>false</c>.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is <c>null</c>.</exception>
        public bool TryGetSecureString(TKey key, out SecureString secString)
        {
            if (IsNull(key))
                throw new ArgumentNullException(nameof(key));

            var hasValue = ContainsKey(key);
            secString = hasValue ? GetSecureString(key) : null;
